...
async function loadServers() {
    try {
        ...
        if (statCount) statCount.innerText = servers.length;

        if (servers.length === 0) {
            if (serversList) serversList.innerHTML = `<div class="no-data">Hələ heç bir server əlavə edilməyib.</div>`;
        ...
        if (serverSelect && serverSelect.getAttribute('data-options-hash') !== currentServerOptionsHash) {
        ...
        if (serversList && serversList.getAttribute('data-render-hash') !== serversHash) {
