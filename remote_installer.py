...
class RemoteInstallerLogic:
    ...
    def update_masterdeploy(self):
        ip = self.gui.ip_entry.get().strip()
        user = self.gui.user_entry.get().strip()
        key_path = self.gui.key_entry.get().strip()
        panel_port = self.gui.panel_port_entry.get().strip() or "3000"
        if not ip or not user or not key_path:
            messagebox.showwarning("Xəta", "Əvvəlcə serverə qoşulun!")
            return

        if not messagebox.askyesno("Təsdiq", f"MasterDeploy ən son versiyaya yenilənsin?\n({ip})"):
            return

        self.log_remote(f"\n--- MasterDeploy YENİLƏNİR ({ip}) ---")

        from tkinter import simpledialog
        token = simpledialog.askstring("GitHub Token", "GitHub Personal Access Token (PAT) daxil edin:", show="*")
        if not token:
            self.log_remote("❌ Yeniləmə ləğv edildi: GitHub Token daxil edilmədi.")
            return
        cmd = f"""
echo '{token}' | sudo docker login ghcr.io -u oauth2 --password-stdin && \
sudo docker pull ghcr.io/kral14/server-repo-rust:latest && \
...
echo '✅ MasterDeploy uğurla yeniləndi!'
"""

        def task():
            ssh_cmd = ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=30",
                       "-i", key_path, f"{user}@{ip}", cmd]
            creationflags = subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
            proc = subprocess.run(ssh_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                  encoding='utf-8', errors='replace', creationflags=creationflags)
            self.log_remote(proc.stdout)
            if proc.returncode == 0:
                self.log_remote("🎉 MasterDeploy uğurla yeniləndi!")
            else:
                self.log_remote("❌ Yeniləmə xətası baş verdi!")
    ...
    def stop_all_services(self):
        if not messagebox.askyesno("Təsdiq", "Uzaq serverdəki BÜTÜN aktiv Docker konteynerlərini dayandırmaq istədiyinizə əminsiniz?"):
            return
        ip = self.gui.ip_entry.get().strip()
        user = self.gui.user_entry.get().strip()
        key_path = self.gui.key_entry.get().strip()
        ...
        self.log_remote(f"\n--- BÜTÜN DOCKER KONTEYNERLƏRİ DAYANDIRILIR ({ip}) ---")

        cmd = "sudo docker stop $(sudo docker ps -q) 2>/dev/null || echo 'Aktiv konteyner tapılmadı!'"

        def task():
            ssh_cmd = ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=10", "-i", key_path, f"{user}@{ip}", cmd]
            creationflags = subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
            proc = subprocess.run(ssh_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding='utf-8', errors='replace', creationflags=creationflags)
            self.log_remote(proc.stdout)
            self.log_remote("✅ Bütün aktiv konteynerlər dayandırıldı!")
    ...
    def test_local_connection(self):
        if os.name == 'nt':
            self.log_local("\n--- Windows Administrator İcazəsi Yoxlanılır ---")
            ...
            def task():
                try:
                    ...
                    if is_admin:
                        self.log_local("✅ ADMİNİSTRATOR İCAZƏSİ TƏSDİQLƏNDİ!")
                        self.gui.root.after(0, lambda: self.gui.toggle_local_buttons(tk.NORMAL))
                        self.gui.root.after(0, lambda: messagebox.showinfo("Uğurlu", "Administrator icazəsi təsdiqləndi!"))
                    else:
                        self.log_local("⚠️ Administrator icazəsi yoxdur. İcazə tələbi göndərilir...")
                        import sys
                        script_path = os.path.abspath(sys.argv[0])
                        params = " ".join([f'"{arg}"' for arg in sys.argv[1:]])
                        
                        self.log_local("🔄 Administrator pəncərəsi açılır, lütfən təsdiqləyin...")
                        ret = ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, f'"{script_path}" {params}', None, 1)
                        
                        if int(ret) > 32:
                            self.log_local("✅ Yeni administrator pəncərəsi açıldı. Bu pəncərəni bağlaya bilərsiniz.")
                            self.gui.root.after(500, self.gui.root.destroy)
                        else:
                            self.log_local("❌ UAC təsdiq edilmədi və ya imtina olundu!")
                            self.gui.root.after(0, lambda: messagebox.showerror("Xəta", "Quraşdırmanı davam etmək üçün Administrator icazəsi verməlisiniz!"))
                except Exception as e:
    ...
    def run_local_task(self, cmd_func, confirm=None):
        if os.name == 'nt':
            ...
            # Buna görə onlara ayrıca metodlar yazırıq (install_local_docker və install_local_wsl)
            # Standart swap və panel qurma kimi Linux əmrlərini isə WSL üzərindən icra edirik
            pass
...
sudo docker stop masterdeploy 2>/dev/null || true;
sudo docker rm masterdeploy 2>/dev/null || true;
echo 'GHCR login olunur...';
echo '{token}' | sudo docker login ghcr.io -u oauth2 --password-stdin;
echo 'GHCR-dan imic çəkilir...';
sudo docker pull ghcr.io/kral14/server-repo-rust:latest;
sudo docker run -d --name masterdeploy --restart always \\
    -p {panel_p}:3000 \\
    -v /var/run/docker.sock:/var/run/docker.sock \\
...
"""
    ...
    def get_cmd_nodejs(self, swap_gb, panel_p, port_p):
        return """
echo 'Node.js və npm yoxlanılır...';
if ! command -v node > /dev/null 2>&1; then
    echo 'Node.js quraşdırılır (Version 20 LTS)...';
    ...
    echo '✅ Node.js və npm uğurla quraşdırıldı!';
    echo 'Node Versiya: '$(node -v);
    echo 'NPM Versiya: '$(npm -v);
else
    echo '✅ Node.js artıq mövcuddur: '$(node -v);
    echo '✅ NPM artıq mövcuddur: '$(npm -v);
fi;
"""
    ...
    def run_remote_task(self, cmd_func, confirm=None):
        ...
        def task():
            ssh_cmd = ["ssh", "-o", "StrictHostKeyChecking=no", "-o", "ConnectTimeout=30", "-i", key_path, f"{user}@{ip}", f"bash -s"]
            try:
                creationflags = subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0
            ...
            try:
                if os.name == 'nt':
                    ...
                    if "sudo " in cmd and sudo_pass:
                        cmd_mod = cmd.replace("sudo ", f"echo '{sudo_pass}' | sudo -S ")
                    else:
                        cmd_mod = cmd
                    proc = subprocess.Popen(
                        ["bash", "-c", cmd_mod],
    ...
    def relaunch_app(self):
        try:
            import sys
            self.log_local("\n🔄 Quraşdırıcı yenidən başladılır...")
            self.save_config()
            os.execv(sys.executable, ['python'] + sys.argv)
        except Exception as e:
            self.log_local(f"❌ Yenidən başlatma xətası: {e}")

    def install_local_docker(self):
        if os.name == 'nt':
            from tkinter import filedialog
            install_dir = filedialog.askdirectory(title="Docker Desktop-un Quraşdırılacağı Qovluğu Seçin (Məsələn: E:\\Docker)")
            if not install_dir:
                messagebox.showwarning("Ləğv edildi", "Qovluq seçilmədiyi üçün Docker quraşdırılması ləğv olundu.")
                return
            ...
            def task():
                try:
                    ...
                    self.log_local("3. Docker böyük imic yaddaşı (WSL data) seçilmiş diskə yönləndirilir...")
                    wsl_data_dir = os.path.join(install_dir, "DockerWSL")
                    os.makedirs(wsl_data_dir, exist_ok=True)
                    
                    subprocess.run(["wsl", "--shutdown"], creationflags=subprocess.CREATE_NO_WINDOW)
                    
                    self.log_local("Mövcud data export olunur...")
                    tar_file = os.path.join(wsl_data_dir, "docker-desktop-data.tar")
                    export_proc = subprocess.run(["wsl", "--export", "docker-desktop-data", tar_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW)
                    
                    if export_proc.returncode == 0:
                        self.log_local("Köhnə registry qeydiyyatı silinir...")
                        subprocess.run(["wsl", "--unregister", "docker-desktop-data"], creationflags=subprocess.CREATE_NO_WINDOW)
                        self.log_local("Yeni diskə idxal (import) edilir...")
                        subprocess.run(["wsl", "--import", "docker-desktop-data", os.path.join(wsl_data_dir, "data"), tar_file, "--version", "2"], check=True, creationflags=subprocess.CREATE_NO_WINDOW)
                        if os.path.exists(tar_file):
                            os.remove(tar_file)
                        self.log_local("✅ Docker yaddaş qovluğu uğurla köçürüldü!")
                    else:
                        self.log_local("ℹ️ Yeni quraşdırma olduğu üçün standart data export-a ehtiyac qalmadı, data qovluğu yaradıldı.")
                    
                    self.log_local("🎉 DOCKER QURAŞDIRILMASI TAMAMLANDI!")
                    self.gui.root.after(0, lambda: messagebox.showinfo("Uğurlu", "Docker Desktop uğurla tənzimləndi!"))
            ...
            self.run_local_task(self.get_cmd_docker)

    def install_local_wsl(self):
        if os.name != 'nt':
            messagebox.showinfo("Məlumat", "Bu funksiya yalnız Windows sistemləri üçündür!")
            return
            
        self.log_local(f"\n--- YERLİ WINDOWS LINUX (WSL) QURAŞDIRILMASI BAŞLADI ---")
        self.gui.toggle_local_buttons(tk.DISABLED)
        
        def task():
            try:
                self.log_local("1. WSL 2 statusu yoxlanılır...")
                wsl_check = subprocess.run(["wsl", "--status"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW)
                if wsl_check.returncode != 0:
                    self.log_local("WSL tapılmadı. WSL 2 quraşdırılması başladılır...")
                    subprocess.run(["wsl", "--install", "--no-launch"], check=True, creationflags=subprocess.CREATE_NO_WINDOW)
                    self.log_local("✅ WSL 2 uğurla aktivləşdirildi! Dəyişikliklərin aktiv olması üçün kompyuteri restart edin.")
                    self.gui.root.after(0, lambda: messagebox.showwarning("Yenidən Başladın", "WSL aktivləşdirildi. Kompyuterinizi restart etdikdən sonra Ubuntu-nu quraşdırın."))
                else:
                    self.log_local("✅ WSL 2 artıq aktivdir.")
                    ...
                    if dist_check.returncode != 0 or b"distrib" in dist_check.stdout.lower() or b"yok" in dist_check.stdout.lower() or len(dist_check.stdout) < 10:
                        ...
                        ps_command = (
                            ...
                            "Write-Host '✅ Ubuntu ugurla qurasdirildi!';"
                        )
                        subprocess.run(["cmd.exe", "/c", "start", "/max", "powershell.exe", "-NoExit", "-Command", ps_command], check=True)
                        self.log_local("⚠️ DİQQƏT: Yeni açılan PowerShell pəncərəsində yüklənmənin və idxalın bitməsini gözləyin, sonra proqramı Relaunch edib davam edin.")
                        self.gui.root.after(0, lambda: self.gui.toggle_local_buttons(tk.NORMAL))
                        return
                    else:
                        self.log_local("✅ Linux (Ubuntu) distribusiya artıq mövcuddur.")
                        self.gui.root.after(0, lambda: messagebox.showinfo("Məlumat", "Linux (Ubuntu) artıq sisteminizdə tam quruludur!"))
            except Exception as e:
                self.log_local(f"❌ XƏTA: {e}")
            self.gui.root.after(0, lambda: self.gui.toggle_local_buttons(tk.NORMAL))
