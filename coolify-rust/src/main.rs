...
async fn setup_server(
    ...
    let cmd = "
        ...
        sudo mkdir -p /data/mini-coolify/apps;
        sudo chown -R $USER:$USER /data/mini-coolify;
        echo 'Setup Complete';
    ";
...
async fn trigger_deployment(
    ...
    tokio::spawn(async move {
        ...
        let prep_cmd = "sudo mkdir -p /data/mini-coolify/apps && sudo chown -R $USER:$USER /data/mini-coolify".to_string();
        ...
        let git_cmd = format!(
            "if [ -d \"/data/mini-coolify/apps/{}\" ]; then cd /data/mini-coolify/apps/{} && git fetch --all && git reset --hard origin/{}; else git clone -b {} {} /data/mini-coolify/apps/{}; fi",
        ...
        let build_cmd = if build_pack_type == "buildpack" {
            ...
            format!(
                "cd /data/mini-coolify/apps/{} && \
                 if [ -f package.json ]; then \
                     ...
                     BUILD_CMD=\"{}\"; [ -z \"$BUILD_CMD\" ] && BUILD_CMD=\"cargo build --release\"; \
                     RUN_CMD=\"{}\"; [ -z \"$RUN_CMD\" ] && RUN_CMD=\"./target/release/{}\"; \
                     rm -f Cargo.lock; \
            ...
            format!(
                "cd /data/mini-coolify/apps/{} && ( [ -f \"{}\" ] || echo -e 'FROM alpine\\nRUN apk add --no-cache curl\\nCMD [\"sleep\", \"3600\"]' > \"{}\" ) && sudo docker build {} -f \"{}\" -t {}:latest .",
